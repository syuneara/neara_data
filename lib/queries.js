...
export function getDateQuery(field, unit, timezone) {
  ...
  if (db === POSTGRESQL) {
    if (timezone) {
      return `to_char(date_trunc('${unit}', ${field} at time zone '${timezone}'), '${
        POSTGRESQL_DATE_FORMATS[unit]
      }')`;
    }
    return `to_char(date_trunc('${unit}', ${field}), '${
      POSTGRESQL_DATE_FORMATS[unit]
    }')`;
  }
  ...
  if (db === MYSQL) {
    if (timezone) {
      ...
      return `DATE_FORMAT(convert_tz(${field},'+00:00','${tz}'), '${
        MYSQL_DATE_FORMATS[unit]
      }')`;
    }
...
export function getPageviewMetrics(
  ...
  if (domain) {
    domainFilter = `and referrer not like $${params.length +
      1} and referrer not like '/%'`;
    params.push(`%://${domain}/%`);
  }
